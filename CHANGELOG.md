...
### 修复
- 修复API地址保存功能无效问题，添加成功和失败的反馈提示
- 修复添加新会话功能无效问题，确保会话创建后正确保存和渲染
- 改进会话标签页样式，明确区分激活和非激活状态

### 优化
- 优化初始化过程中的会话加载逻辑，增强系统稳定性
- 改进API地址处理逻辑，增强URL验证和代理判断
...
### 修复
- 修复API主机保存按钮ID不匹配问题，确保保存按钮能正常工作
- 修复API地址保存后需要刷新页面才能生效的问题，现在保存后立即生效并重新加载模型列表
- 修复代码高亮问题，添加对代码块类名的检查，确保只对有语言标识的代码块进行高亮处理

### 优化
- 更新API配置提示文本，明确告知用户配置已立即生效
